#!/usr/bin/env python3
"""
LSTM with Route-Aware Fixes, Last Stop Logic, 
Realistic Boarding Constraints, and Night-Time Logic (1-4 AM Only)

Key features:
1. Preserves stop_mapping from training to prediction
2. Handles data type consistency for stop_id
3. Better debugging for unknown stops
4. Graceful handling of missing stops
5. Route-aware adjustments based on stop classifications
6. LOGICAL CONSTRAINTS: Alighting cannot exceed current load
7. LAST STOP LOGIC: At last stops, boarding=0 and alighting=current_load
8. REALISTIC BOARDING CONSTRAINTS: Driver discretion and passenger behavior
9. NIGHT-TIME LOGIC: 1-4 AM specific constraints (reduced caps: boarding≤5, load≤15)
10. Comprehensive fix reporting
"""
...
class lstm_model:
    ...
    def train_with_pattern_validation(self, train_X, train_y, val_X, val_y, epochs=10):
    ...
    def _apply_realistic_boarding_constraints(self, current_load, predicted_boarding, capacity, max_occupancy=None):
        ...
        4. Random maximum occupancy cap between 1.5-1.6
        """
    ...
    def _calculate_loads_with_realistic_and_night_constraints(self, df):
        ...
        print(f"\nFINAL OCCUPANCY STATISTICS WITH NIGHT-TIME AWARENESS AND OCCUPANCY CAPS (1.5-1.6):")
            ...
            for idx in bus_data.index:
                ...
                if is_last:
                    ...
                    constrained_boarding = self._apply_realistic_boarding_constraints(
                        current_load, predicted_boarding, capacity, bus_max_occupancy[bus_id]
                    )
    ...
    def predict_with_patterns(self, historical_df, future_df, apply_route_adjustments=True, apply_realistic_constraints=True, apply_night_constraints=True):
        ...
        # Track last stop applications and rush hour boosts
    ...
    def _validate_prediction_patterns(self, predictions_df):
        ...
        # Check if rush hours are preserved (with 1.2x boost)
        rush_hours = [7, 8, 9, 17, 18, 19]
        ...
        if len(pred_hourly) > 0:
            ...
            if off_peak_avg > 0:
                ...
                print(f"Rush hour ratio: {ratio:.2f} (should be 2.4+ with 1.2x boost)")
                if ratio > 2.4:
                    print("✅ Excellent rush hour pattern with boost!")
                elif ratio > 2.0:
                    print("📊 Good rush hour pattern with boost")
                elif ratio > 1.5:
                    print("📊 Moderate rush hour pattern preservation")
                else:
                    print("⚠️  Weak rush hour patterns")
            
            print(f"Predicted rush hour avg (with 1.2x boost): {rush_avg:.1f}")
...
def main():
    ...
    try:
        ...
        # Create model
        model = lstm_model(sequence_length=args.sequence)
