...
class HybridModel:
    ...
    def predict_hybrid(self, historical_df, future_df, apply_route_adjustments=True, 
        ...
        # Count rush hour records for reporting
        future_df_temp = future_df.copy()
        future_df_temp['datetime'] = pd.to_datetime(future_df_temp['datetime'])
        future_df_temp['hour'] = future_df_temp['datetime'].dt.hour
        rush_hours = [7, 8, 9, 17, 18, 19]
        rush_hour_records = (future_df_temp['hour'].isin(rush_hours)).sum()
        total_records = len(future_df_temp)
        
        print(f"Rush hour analysis (1.2x boarding boost applied by individual models):")
        print(f"  Rush hour records: {rush_hour_records} ({rush_hour_records/total_records*100:.1f}%)")
        print(f"  Normal hour records: {total_records - rush_hour_records} ({(total_records - rush_hour_records)/total_records*100:.1f}%)")
