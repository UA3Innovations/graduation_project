...
from data_models import BusTransitData, LineScheduleEntry, BusAssignment, Bus
...
class ScheduleGenerator:
    ...
    def _generate_departures_for_period(self, date: datetime, period: Tuple[int, int], 
        ...
        return departures
# Add this method to ScheduleGenerator class
    
    def load_historical_bus_constraints(self, historical_file: str):
        """Load exact bus usage from historical data"""
        import pandas as pd
        historical_df = pd.read_csv(historical_file)
        
        # Set EXACT limits from historical data
        self.total_bus_limit = historical_df['bus_id'].nunique()  # Should be 65
        
        for line_id in historical_df['line_id'].unique():
            line_data = historical_df[historical_df['line_id'] == line_id]
            self.historical_bus_limits[line_id] = line_data['bus_id'].nunique()
        
        print(f"🚌 Hard bus limit set: {self.total_bus_limit} buses total")
