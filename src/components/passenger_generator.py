...
from ..core.data_models import BusTransitData, PassengerGroup, Stop, Bus
...
class PassengerGenerator:
    ...
    def __init__(self, data: BusTransitData, network: TransitNetwork):
        ...
        for stop_id, stop in self.data.stops.items():
            ...
            # Determine initial waiting passenger count based on stop importance
            if stop.is_special_location:
                base_waiting = random.randint(15, 45)  # Increased from 8-25
            else:
                base_waiting = random.randint(5, 20)   # Increased from 2-10
            ...
            if has_crowded_line:
                base_waiting *= random.uniform(2.0, 3.0)  # Increased from 1.5-2.2
    ...
    def get_time_factor(self, dt: datetime, line_id: str, stop_id: int) -> float:
        ...
        # Apply time of day factors - DRAMATICALLY INCREASED FOR HEAVY OVERCROWDING
        if is_morning_rush and not (is_weekend or is_holiday):
            # Morning rush hour - EXTREME demand increase for optimization testing
            if line.is_crowded:
                factor *= 6.0 if stop.is_special_location else 5.0  # Increased from 3.5/3.0
            else:
                factor *= 4.5 if stop.is_special_location else 3.8  # Increased from 2.8/2.3
        elif is_evening_rush and not (is_weekend or is_holiday):
            # Evening rush hour - EXTREME demand increase
            if line.is_crowded:
                factor *= 5.5 if stop.is_special_location else 4.5  # Increased from 3.2/2.7
            else:
                factor *= 4.0 if stop.is_special_location else 3.5  # Increased from 2.5/2.1
        elif hour < self.network.OPERATIONAL_HOURS[0] or hour >= self.network.OPERATIONAL_HOURS[1]:
            factor *= 0.2  # Very early morning or late night
        elif 9 <= hour < 12 or 13 <= hour < 16:
            factor *= 1.2  # Increased regular working hours from 0.7
        elif 12 <= hour < 13:
            factor *= 2.0  # Increased lunch time from 1.2
        elif 19 <= hour < 21:
            factor *= 1.8  # Increased evening activities from 1.0
        
        # Weekend and holiday factors
        if is_weekend:
            if 10 <= hour < 20:  # Peak hours shift later on weekends
                factor *= 1.4  # Increased from 0.9
            else:
                factor *= 0.8  # Increased from 0.5
                
        if is_holiday:
            factor *= 1.0  # Increased from 0.6 - holidays still busy for optimization testing
    ...
    def generate_passengers(self, dt: datetime, line_id: str, stop_id: int, 
        ...
        # Get stop baseline boarding rate and convert to per-minute
        base_minute_rate = stop.base_boarding_rate / 60.0
        ...
        # Apply rush hour and crowded line effects
        weekday = dt.weekday()
        is_rush_hour = ((7 <= hour < 10) or (16 <= hour < 20)) and weekday < 5
        
        # EXTREME variations in passenger demand for genetic algorithm optimization
        if is_rush_hour and line_id in self.network.CROWDED_LINES:
            # DRASTICALLY higher demand during rush hour for crowded lines (6-10x normal)
            adjusted_boarding_rate *= random.uniform(6.0, 10.0)  # Increased from 3.0-5.0
            
            # Extra spike on specific days for some lines
            if line_id in ['101', '105-2'] and weekday in [1, 3]:  # Tuesday, Thursday
                adjusted_boarding_rate *= random.uniform(2.0, 3.0)  # Increased from 1.5-2.0
                
        elif is_rush_hour and line_id in self.network.UNSTABLE_LINES:
            # Unstable lines have EXTREME unpredictable demand during rush hour
            volatility = random.random()
            if volatility > 0.5:  # Increased chance from 60% to 50%
                adjusted_boarding_rate *= random.uniform(5.0, 8.0)  # Increased from 3.0-4.0
            else:  # Otherwise strong increase
                adjusted_boarding_rate *= random.uniform(3.0, 4.5)  # Increased from 1.5-2.0
                
        elif is_rush_hour:
            # Regular lines - STRONG increase during rush hour for optimization testing
            adjusted_boarding_rate *= random.uniform(3.5, 5.0)  # Increased from 1.8-2.5
            
        elif line_id in self.network.CROWDED_LINES:
            # Crowded lines have MUCH higher demand even outside rush hour
            adjusted_boarding_rate *= random.uniform(2.5, 3.5)  # Increased from 1.5-2.0
            
        elif line_id in self.network.UNDERUTILIZED_LINES:
            # Underutilized lines still have some increased demand for testing
            adjusted_boarding_rate *= random.uniform(0.8, 1.2)  # Increased from 0.5-0.8
            
            # More frequent unexpected bursts of passengers
            if random.random() < 0.1:  # Increased from 5% to 10% chance
                adjusted_boarding_rate *= random.uniform(4.0, 7.0)  # Increased from 3.0-5.0
        ...
        if stop.is_special_location:
            potential_boarding = min(potential_boarding, 200)  # Increased from 120 for special locations
        else:
            potential_boarding = min(potential_boarding, 120)  # Increased from 70 for regular stops
        ...
        if not hasattr(bus, 'overcrowding_tolerance'):
            ...
            strict_driver = random.random() < 0.25  # Reduced from 30% to 25% for strict drivers
            if strict_driver:
                tolerance = random.uniform(1.4, 1.6)  # Increased from 1.3-1.45
            else:
                tolerance = random.uniform(1.7, 2.0)  # Increased from 1.5-1.7
        ...
        if current_load >= bus.capacity:
            ...
            if current_load >= max_capacity * 0.95:  # Near absolute maximum
                # Driver stops opening doors at very high occupancy
                driver_frustration = random.random()
                if driver_frustration > 0.4:  # 60% chance driver simply refuses more boardings
                    available_capacity = 0
                else:
                    # Maybe 1-2 can squeeze in at most
                    available_capacity = int(random.randint(0, 2) * overcrowding_factor)
            elif current_load >= max_capacity * 0.85:  # Very high occupancy
                # Almost no one can board
                available_capacity = int(random.randint(0, 4) * overcrowding_factor)
            elif current_load >= bus.capacity * 1.4:  # Significant overcrowding
                # Very few can board
                available_capacity = int(random.randint(2, 6) * overcrowding_factor)
            else:  # Moderate overcrowding
                # Some can still board
                available_capacity = int(random.randint(5, 12) * overcrowding_factor)
        else:
    ...
    def accumulate_waiting_passengers(self, dt: datetime, time_step: int) -> None:
        ...
        for stop_id, stop in self.data.stops.items():
            ...
            if stop.is_special_location:
                base_rate = random.uniform(0.4, 1.8)  # Increased from 0.2-1.0 passengers per minute
            else:
                base_rate = random.uniform(0.1, 0.6)  # Increased from 0.05-0.3 passengers per minute
            ...
            if is_rush_hour and not (is_weekend or is_holiday):
                base_rate *= random.uniform(4.0, 7.0)  # Increased from 3.0-5.0
            elif is_weekend:
