...
class TransitNetwork:
    ...
    # Time periods - EXTENDED RUSH HOURS for more optimization stress
    RUSH_HOURS_MORNING = [(6, 11)]  # Extended from (7, 10) to (6, 11)
    RUSH_HOURS_EVENING = [(15, 21)]  # Extended from (16, 20) to (15, 21)
    ...
    def _process_lines(self, df: pd.DataFrame) -> None:
        ...
        for line_id in line_ids:
            ...
            # Determine line classification and popularity factor based on actual data
            if line_id in self.CROWDED_LINES:
                is_crowded = True
                popularity = random.uniform(1.8, 2.4)  # High demand lines
            elif line_id in self.UNDERUTILIZED_LINES:
                is_crowded = False
                popularity = random.uniform(0.4, 0.7)  # Low demand lines
            elif line_id in self.UNSTABLE_LINES:
                is_crowded = True if random.random() < 0.4 else False  # Sometimes crowded
                popularity = random.uniform(0.9, 1.6)  # Unpredictable demand
            else:
                is_crowded = False
                popularity = random.uniform(0.8, 1.2)  # Normal demand
    ...
    def _initialize_passenger_profiles(self) -> None:
        ...
        for stop_id, stop in self.data.stops.items():
            ...
            # Base boarding rate (passengers per hour) - DRAMATICALLY INCREASED for optimization testing
            # Much higher values to stress-test the genetic algorithm
            if stop.is_special_location:
                ...
                if any(loc in stop.stop_name for loc in ["Kızılay", "AŞTİ", "Aşti", "Opera"]):
                    # Major transit hubs get EXTREME rates for optimization testing
                    base_boarding = random.randint(120, 200)  # INCREASED from 50-100 passengers per hour
                    ...
                    # Universities have very high time-dependent patterns
                    base_boarding = random.randint(80, 150)  # INCREASED from 40-75 passengers per hour
                    transfer_likelihood = random.uniform(0.3, 0.45)
                elif any(loc in stop.stop_name for loc in ["Gazi Hastanesi", "Teknokent", "Avm", "Taurus"]):
                    # Malls get much higher boarding for optimization stress
                    base_boarding = random.randint(80, 150)  # INCREASED from 40-75 passengers per hour
                    transfer_likelihood = random.uniform(0.35, 0.55)
                else:
                    # Other special locations get increased rates
                    base_boarding = random.randint(60, 120)  # INCREASED from 25-55 passengers per hour
                    transfer_likelihood = random.uniform(0.35, 0.5)
            else:
                # Regular stops with MUCH higher boarding for optimization testing
                if lines_serving_stop > 1:
                    # Stops served by multiple lines
                    base_boarding = random.randint(40, 80)  # INCREASED from 18-40 passengers per hour
                    transfer_likelihood = random.uniform(0.2, 0.35)
                else:
                    # Single line stops - significant increase
                    base_boarding = random.randint(25, 50)   # INCREASED from 10-25 passengers per hour
