...
try:
    from core.data_models import BusTransitData, SimulationConfig, LineScheduleEntry, BusAssignment
    ...
    # Fallback to compartmentalized imports
...
class ScheduleChromosome:
    ...
    def __repr__(self):
        ...
        return f"ScheduleChromosome(line={self.line_id}, departures={len(self.departure_times)}, fitness={fitness_str})"
...
class GeneticScheduleOptimizer:
    ...
    def __init__(self, config: OptimizationConfig = None, stops_file: str = None):
        ...
        self.stops_file = stops_file or "ankara_bus_stops.csv"
        self.base_data = None
        self.network = None
        self.line_ids = []
...
def run_optimization(stops_file: str = "ankara_bus_stops.csv", 
                    optimization_date: str = "2025-06-02", 
                    output_file: str = "optimized_schedule.csv",
                    target_lines: Optional[List[str]] = None):
    """Run the complete schedule optimization process using simulation"""
    
    print("🚌 Bus Schedule Optimization with Genetic Algorithm")
    print("=" * 60)
    
    # Initialize optimizer with simulation-based evaluation
    config = OptimizationConfig(
        population_size=20,  # Smaller for faster simulation-based evaluation
        generations=30,      # Fewer generations due to expensive fitness evaluation
    ...
    if target_lines is None:
        target_lines = optimizer.line_ids[:3]  # Optimize first 3 lines for testing
...
def quick_test_optimization():
    ...
    optimizer = GeneticScheduleOptimizer(config, "ankara_bus_stops.csv")
